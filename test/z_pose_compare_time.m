...
timec_s_abs(k, 3) = toc(start);

start = tic();

[Rb_6, tb_6] = pxp_x6(eye(3),[0;0;0], pat1, pat2);

timec_s_abs(k, 4) = toc(start);

start = tic(); % TIC

[Rb_10, tb_10] = pxp_v2(eye(3), [0;0;0], pat1, pat2);

timec_s_abs(k, 5) = toc(start); % TOC
...
save(['results_' name '_' num2str(N) '_' num2str(lines) '_time.mat'], 'timec_s_abs', 'error_z_neg', 'error_R_ang', 'error_R_abs', 'error_t_abs', 'error_t_rel', 'error_t_ang');
